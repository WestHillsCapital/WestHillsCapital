...
-   **DocuFill shared field library:** Global reusable field definitions live in `docufill_fields` and are exposed through internal DocuFill bootstrap/library endpoints. Package fields can reference `libraryFieldId`, keeping per-package document mappings and presentation settings separate while hydrating shared label/type/source/options/sensitivity/required/validation rules into package editors, interviews, and packet generation.
