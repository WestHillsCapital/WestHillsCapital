...
- **Custom Domain (Pro/Enterprise):** Organizations on Pro or Enterprise plans can configure a custom subdomain (e.g. `forms.acme.com`) for customer-facing interview links. Settings page has a "Custom domain" section with DNS CNAME instructions, a Verify button (real DNS lookup via Node `dns.promises.resolveCname`), and status badges (Unverified / Verifying / Active / Error). Domain stored in `accounts.custom_domain` + `custom_domain_status` + `custom_domain_verified_at`. API routes: `GET/PUT /api/v1/product/settings/custom-domain`, `POST /api/v1/product/settings/custom-domain/verify`. When active, new interview links and send-link emails use the custom domain as origin. Free plan users see an upgrade prompt.
