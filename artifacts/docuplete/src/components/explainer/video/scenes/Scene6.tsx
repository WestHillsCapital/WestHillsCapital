...
export function Scene6() {
  ...
  return (
    ...
    >
      <motion.h2
        className="text-[3.5vw] font-black text-white text-center leading-tight mb-3"
      ...
      >
        Docuplete{' '}
        <span style={{ color: '#1B4FD8' }}>auto-fills</span>
        {' '}every document.
      </motion.h2>

      <motion.p
        className="text-white/50 text-lg mb-10 text-center"
        ...
        {DOCS.map((doc, i) => (
          ...
          >
            <div className="px-5 pt-5 pb-3 border-b border-gray-100">
              <div className="w-3 h-3 rounded-full mb-2" style={{ backgroundColor: doc.color }} />
              <h4 className="text-sm font-bold text-gray-800 leading-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{doc.name}</h4>
              <p className="text-xs text-gray-400" style={{ fontFamily: "'DM Sans', sans-serif" }}>Page {doc.page}</p>
            </div>

            <div className="p-5 space-y-3">
              {doc.filled.map((line, j) => (
                <div key={line}>
                  <div className="text-xs text-gray-400 mb-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>{line.split(':')[0]}</div>
                  <motion.div
