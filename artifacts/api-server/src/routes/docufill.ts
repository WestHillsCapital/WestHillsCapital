...
async function requireAccountNotPurged(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const row = rows[0];
    // Allow active/trialing subscribers through even if data_purged_at is set —
    // data_purged_at is an immutable audit record; subscription_status is the
    // authoritative access signal for reactivated accounts.
    const isActiveSub = row?.subscription_status === "active" || row?.subscription_status === "trialing";
    if (row?.data_purged_at != null && !isActiveSub) {
      res.status(410).json({
...
function normalizeFieldGroupRow(
...
// GET /field-library/groups — list all field groups for this account (includes usagePackages for consistency with bootstrap)
router.get("/field-library/groups", requireAdminRole, async (req, res) => {
  try {
    ...
    const fieldMap = new Map(fieldLibrary.map((f) => [f.id, f]));
    const usageByGroupId = new Map<number, Array<{ id: number; name: string }>>();
    for (const r of (usageRows.rows as Array<{ group_id: number; package_id: number; package_name: string }>)) {
      const list = usageByGroupId.get(r.group_id) ?? [];
      list.push({ id: r.package_id, name: r.package_name });
      usageByGroupId.set(r.group_id, list);
    }
    res.json({
      fieldGroups: (rows as Array<Record<string, unknown>>).map((row) =>
        normalizeFieldGroupRow(row, fieldMap, usageByGroupId.get(row.id as number) ?? []),
      ),
    });
  } catch (err) {
...
router.post("/field-library/groups", requireAdminRole, async (req, res) => {
  try {
    ...
    // Validate fieldIds — keep only IDs that exist in this account's library (deduplicated, order preserved)
    const validLibraryIds = new Set(fieldLibrary.map((f) => f.id));
    const rawIds = Array.isArray(body.fieldIds) ? body.fieldIds.filter((id) => typeof id === "string") : [];
...
router.patch("/field-library/groups/:id", requireAdminRole, async (req, res) => {
  try {
    ...
    // Fetch field library once for both validation and response normalization
    const fieldLibrary = body.fieldIds !== undefined ? await getFieldLibrary(db, accountId) : null;
    if (body.fieldIds !== undefined) {
      const validLibraryIds = new Set((fieldLibrary ?? []).map((f) => f.id));
      const rawIds = Array.isArray(body.fieldIds) ? body.fieldIds.filter((i) => typeof i === "string") : [];
      const ids = [...new Set(rawIds)].filter((id) => validLibraryIds.has(id));
    ...
    const resolvedLibrary = fieldLibrary ?? await getFieldLibrary(db, accountId);
...
router.post("/sessions/:token/generate", requireMemberRole, async (req, res) => {
  try {
    ...
    // When queue is unavailable (e.g. CI, Redis down) fall back to synchronous
    // generation so tests and degraded-mode production still work.
    if (!isQueueEnabled()) {
 ...
 *
 * When the job queue is available, returns 202 { jobId, status: "pending" }
 * and processes asynchronously in the PDF worker.  When the queue is
...
publicDocufillRouter.post("/sessions/:token/preview-pdf", async (req, res) => {
  try {
    ...
    if (!res.headersSent) {
      if (isConnectivityError) {
        // Queue connectivity error: attempt synchronous fallback
        try {
          const db = getDb();
          const session = await getSession(req.params.token, db);
          if (session && session.status !== "generated" && session.status !== "voided") {
            const pdfBuffer = await buildPacketPdfBuffer(session, db);
            res.setHeader("Content-Type", "application/pdf");
            res.setHeader("Content-Disposition", `inline; filename=preview-${req.params.token}.pdf`);
            res.setHeader("Content-Length", String(pdfBuffer.length));
        ...
        res.status(503).json({
          error: "Preview generation is temporarily unavailable. Please try again shortly.",
          code: "queue_unavailable",
        });
      } else {
...
publicDocufillRouter.get("/sessions/:token/preview-pdf/status", async (req, res) => {
  try {
    ...
    if (state === "failed") {
      res.json({ status: "failed", error: "Preview generation failed. Please try again." });
      return;
    }
    res.json({ status: state === "active" ? "processing" : "pending" });
  } catch (err) {
  ...
  const worker = createQueueWorker<GeneratePdfJobPayload, void>(
    ...
    async (job) => {
      ...
      if (job.data.type === "preview") {
        const previewBuffer = await buildPacketPdfBuffer(session, db, {
          signatureImage: esignSignatureImage ?? undefined,
          signerName:     esignSignerName     ?? undefined,
          initialsImage:  esignInitialsImage  ?? undefined,
        });
        try {
          await objectStorage.uploadBuffer(
            `previews/${sessionToken}.pdf`,
            previewBuffer,
            "application/pdf",
          );
          logger.info({ sessionToken }, "[GeneratePdf] Preview PDF stored in object storage");
        } catch (storageErr) {
          logger.warn({ storageErr, sessionToken }, "[GeneratePdf] Preview storage upload failed (non-fatal)");
        }
        logger.info({ jobId: job.id, sessionToken }, "[GeneratePdf] Preview job completed");
        return;
      }
