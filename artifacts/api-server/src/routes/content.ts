...
router.post("/draft", async (req, res): Promise<void> => {
  ...
  // We write whitespace chunks while Anthropic generates, then flush the
  // final JSON. JSON.parse on the client ignores leading whitespace.
  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      // Keep the Railway proxy from timing out on an idle connection
      res.write(" ");
    }
