...
- Auto-fill (copyFrom): a field can be configured to automatically copy its value from another field when a trigger field equals a specific value. Example: when "Beneficiary Relationship" equals "Spouse", all beneficiary address fields automatically populate from the primary account holder address fields. Configure in the field editor under "Auto-fill from another field when" — set the source field (the field to copy from), the trigger field (the field whose value drives the copy), and the trigger value (the value that activates the copy). The auto-fill fires instantly in the interview as soon as the trigger condition is met. Users can still edit the auto-filled value after it populates.
