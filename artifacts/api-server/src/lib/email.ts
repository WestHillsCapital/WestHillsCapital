...
export async function sendBookingNotification(params: {
  ...
  if (!ADMIN_EMAIL) {
    logger.warn("[Email] ADMIN_EMAIL not set — admin booking notification skipped");
    return;
  }
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px;">
        ...
        <table style="border-collapse:collapse;width:100%;font-size:14px;">
          ...
          <tr><td style="padding:6px 0;color:#6b7280;">Phone</td><td style="padding:6px 0;">${params.phone}</td></tr>
          ...
          <tr><td style="padding:6px 0;color:#6b7280;">Structure</td><td style="padding:6px 0;">${allocationLabels[params.allocationType] ?? params.allocationType}</td></tr>
          <tr><td style="padding:6px 0;color:#6b7280;">Allocation</td><td style="padding:6px 0;">${rangeLabels[params.allocationRange] ?? params.allocationRange}</td></tr>
          <tr><td style="padding:6px 0;color:#6b7280;">Timeline</td><td style="padding:6px 0;">${timelineLabels[params.timeline] ?? params.timeline}</td></tr>
        </table>
      </div>
    `,
  });
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px;">
        ...
        <p style="color:#374151;">Your allocation discussion with West Hills Capital has been scheduled.</p>
        <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:16px;margin:20px 0;">
          <p style="margin:0 0 8px;"><strong>Date:</strong> ${params.dayLabel}</p>
          <p style="margin:0 0 8px;"><strong>Time:</strong> ${params.timeLabel}</p>
          <p style="margin:0;"><strong>Confirmation ID:</strong> ${params.confirmationId}</p>
        </div>
        <p style="color:#374151;">We will call you from <strong>(800) 867-6768</strong> at your scheduled time. Please have this confirmation ID available.</p>
        <p style="color:#374151;">During the call we will review your intended allocation, confirm current pricing, and discuss execution steps.</p>
        ...
        <p style="color:#6b7280;font-size:13px;">West Hills Capital &nbsp;|&nbsp; (800) 867-6768</p>
      </div>
    `,
  });
}
