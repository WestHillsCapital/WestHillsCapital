...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    icon: "📚",
    title: "Field Library",
    summary: "The Library tab stores shared field definitions and organizational data your whole team uses across packages. It has five sub-tabs: Fields, Field Groups, Types, Groups, and Compliance.",
    items: [
      { label: "Fields tab — collapsible rows", detail: "All shared field definitions are listed as compact rows. Click any row to expand an inline edit drawer directly below it. The drawer shows all field settings in one place — label, type, validation, sort order, prefill source, compliance tags, and more. Prefill Source and Sort Order sit side-by-side in the drawer body for easy comparison. Collapse the row by clicking it again or clicking a different row." },
      { label: "Fields tab — saving and deleting", detail: "Inside an expanded field row, the Delete and Save buttons live in an isolated shaded footer bar at the bottom of the drawer, visually separated from the editing area. This prevents accidental clicks. The Save button stays gold; Delete is red. Both show a spinner and confirmation state." },
      { label: "Fields tab — version history", detail: "Open any field drawer and click the History tab to see every prior save. Each version entry shows a labeled diff — for example, 'Label: \"Client Name\" → \"Applicant Full Name\"'. Click 'Restore this version' on any entry to roll back immediately. The rollback applies to all future sessions; submitted sessions are unaffected." },
      { label: "Fields tab — analytics", detail: "The Analytics tab inside each field drawer shows how many sessions have collected data for this field. Use it before making changes to gauge the impact of edits on active packages." },
      { label: "Field Groups tab", detail: "A Field Group bundles related library fields under one name (e.g. 'Client Address' groups Street, City, State, ZIP). Cards are equal-height and show two badges: a blue pill with the field count and a green pill listing every package that uses the group (or a gray 'No packages' pill if unused). Click '+ Add Group' in the header to create a new group. In the mapper's field list, use 'Browse groups' to add all fields in a group to a package at once." },
      { label: "Types tab", detail: "Transaction Types (e.g. 'New Account', 'IRA Transfer', 'Withdrawal') appear as a compact single-row ledger — one type per line with label, sort number, assigned groups as tag strips, and an Active toggle. Click the label or sort number to edit in place; the row auto-saves when you move focus away. A spinning → ✓ indicator confirms the save. Use the ⋮ menu at the far right to see the type's scope slug or delete it." },
      { label: "Groups tab", detail: "Recipient Groups (e.g. 'Equity Trust Company', 'Depository') appear as cards. Each card has a name input at the top, then a three-column row for PHONE NUMBER, EMAIL ADDRESS, and STATUS (Active checkbox) — all labeled with compact uppercase metadata headers so you can scan contact info at a glance. The Delete and Save actions live in a dedicated shaded footer bar at the bottom of each card, matching the same pattern as the Fields drawer." },
      { label: "Compliance tab — reading the audit", detail: "Click the Compliance sub-tab and press Refresh to run the audit. Each package gets a card. Compliant packages show a soft green background with a '✓ Compliant' badge. Packages that are missing one or more required-tagged fields show an amber background with a '⚠️ Non-Compliant · N missing' badge. Inside each card, Present and Missing field lists sit side-by-side (max 600 px wide) so you can compare them without scrolling." },
      { label: "Compliance tab — exporting", detail: "Once an audit is loaded, an 'Export CSV' button appears next to Refresh. Click it to download a spreadsheet with one row per package showing the package name, status, gap count, and the full present/missing field lists. Use this for internal reviews, regulator submissions, or sharing results outside the app." },
      { label: "Importing a library field into a package", detail: "From any package's field list in the mapper, click 'Browse groups' or search for the field by name. Library fields show a chain-link icon. When imported, the package maintains a live link — label, validation, and options changes in the library propagate to the package for new sessions." },
      ...
      { label: "Export / Import", detail: "Use Export → JSON or Export → CSV to download the full field library for backup or migration. Use Import to load definitions from a previously exported file. The import preview marks each row as Update (blue), No change (grey), or New (green) before writing anything." },
    ],
  },
  {
    id: "esign",
