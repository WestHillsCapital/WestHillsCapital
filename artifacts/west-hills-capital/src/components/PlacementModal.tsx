...
export interface PlacementModalProps {
  ...
  onClose: () => void;
  onSetModalPos: (pos: { x: number; y: number }) => void;
...
export function PlacementModal({
  ...
  function startModalDrag(e: React.PointerEvent<HTMLElement>) {
    ...
    function onMove(ev: PointerEvent) {
      onSetModalPos({ x: startLeft + (ev.clientX - startX), y: startTop + (ev.clientY - startY) });
    }
          ...
          {field && (
            <div className="flex items-center gap-1.5">
              <input
                ...
                className={`flex-1 border border-[#E2E8F0] rounded px-2.5 py-1.5 text-xs text-[#0F1C3F] focus:outline-none ${isSystemEsignFieldId(field.id) ? "bg-[#F8FAFC] text-[#6B7A99] cursor-default" : "focus:ring-1 focus:ring-[#C49A38] focus:border-[#C49A38]"}`}
