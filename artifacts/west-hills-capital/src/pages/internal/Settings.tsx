...
function FieldLibraryInheritanceSection({
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`${SETTINGS_BASE}/inheritance`, { headers: { ...getAuthHeaders() } })
      .then(async (r) => {
        const data = await r.json() as InheritanceInfo & { error?: string };
        if (!r.ok) { setLoadError(data.error ?? "Failed to load inheritance info"); return; }
        setInfo(data);
      })
      .catch(() => setLoadError("Failed to load inheritance info"))
      .finally(() => setIsLoading(false));
  }, [getAuthHeaders]);
  ...
  return (
    <section className="bg-white rounded-xl border border-[#DDD5C4] divide-y divide-[#EFE8D8]">
      <div className="px-6 py-4 flex items-center gap-2">
        <svg className="w-4 h-4 text-[#1B4FD8] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
        <div>
          <h2 className="text-base font-semibold text-[#0F1C3F]">Field Library Sharing</h2>
          <p className="text-xs text-[#6B7A99] mt-0.5">
            {canManageChildren
      ...
      {parent && (
        <div className="px-6 py-4">
          <p className="text-xs font-medium text-[#0F1C3F] mb-1">Inheriting from</p>
          <div className="flex items-center gap-2 rounded-lg border border-[#C8D7F5] bg-[#F3F6FE] px-3 py-2">
            <svg className="w-3.5 h-3.5 text-[#1B4FD8] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
            <span className="text-sm font-medium text-[#0F1C3F]">{parent.name}</span>
      ...
      {canManageChildren && (
        <>
          <div className="px-6 py-4">
            <p className="text-xs font-medium text-[#0F1C3F] mb-2">
              Linked child accounts {children.length > 0 && <span className="text-[#8A9BB8] font-normal">({children.length})</span>}
            </p>
            {children.length === 0 ? (
              <p className="text-[11px] text-[#8A9BB8]">No child accounts linked yet. Link an account below to share your field library with them.</p>
            ) : (
              <ul className="space-y-1.5">
                {children.map((child) => (
                  <li key={child.id} className="flex items-center justify-between gap-3 rounded-lg border border-[#EFE8D8] bg-[#F8F6F0] px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <svg className="w-3.5 h-3.5 text-[#6B7A99] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                      <span className="text-sm text-[#0F1C3F] truncate">{child.name}</span>
                      <span className="text-[10px] text-[#8A9BB8]">{child.slug}</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => void handleUnlink(child.id, child.name)}
                      disabled={unlinkingId === child.id}
                      className="shrink-0 text-[11px] text-red-500 hover:text-red-700 disabled:opacity-50 transition-colors"
                    >
                      {unlinkingId === child.id ? "Removing…" : "Remove"}
                    </button>
                  </li>
                ))}
              </ul>
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F0]">
      <div className="max-w-4xl mx-auto px-4 py-10 space-y-8">
        ...
        {/* Field Library Sub-accounts */}
        <FieldLibraryInheritanceSection getAuthHeaders={getAuthHeaders} org={org} />
      </div>
    </div>
  );
}
